...
static HomeLocation home_locations[] = { { HOMEDIR, ".sauerbraten" } };
...
typedef struct {
    ...
    GFileMonitor *auth_cfg_monitor;
    GFileMonitor *autoexec_cfg_monitor;
...
GdkPixbuf *get_stock_icon(GtkIconTheme *icon_theme, const char *icon_name,
        int size = 24) {
    GError *error = NULL;
    GdkPixbuf *return_pixbuf = NULL;

    return_pixbuf = gdk_pixbuf_new_from_file_at_size(icon_name, size /*px*/,
            size /*px*/, &error);
    if (!return_pixbuf) {
...
void add_row(AuthManager *authManager, bool enabled, const char *authname,
        const char *privkey, const char *domain, FileBuffer *config_buffer,
        int config_line) {
    GtkTreeIter current_entry;
    gtk_list_store_append(authManager->key_store, &current_entry);
    gtk_list_store_set(authManager->key_store, &current_entry, COL_ENABLED,
            enabled, COL_AUTHNAME, authname, COL_PRIVKEY, privkey, COL_DOMAIN,
            domain, COL_ACTION_INFO, authManager->info_icon_pixbuf,
            COL_ACTION_EDIT, authManager->edit_icon_pixbuf, COL_ACTION_DELETE,
            authManager->delete_icon_pixbuf, COL_CONFIG_BUFFER,
            (gpointer) config_buffer, COL_CONFIG_LINE, config_line, -1);
}
